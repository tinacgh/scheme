;; This file contains an MIT-Scheme version of the object-oriented
...
;;
;; This is the MIT Scheme specific version.  The only change from the
;; system-independent version is the definition of eval-globally.
;;
;; Revision 1.3 as of 1999/06/15 21:53:32
...
; and eval each of the individual definitions is because of an
; unfortunate aspect of how MIT Scheme works,
; namely that all the names being defined within a begin are first
...
(define eval-globally
  (lambda (exp)
    (if (and (pair? exp)
             (eq? (car exp) 'begin))
        (for-each eval-globally (cdr exp))
